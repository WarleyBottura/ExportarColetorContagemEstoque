...
-----------------------------------------------------------------------------
- Novo layout: área superior compartilhada (Planilha **ou** DB), logo abaixo seleção de colunas,
  depois linha de formatação + botões, e por fim a prévia.
...
NL = os.linesep   # usado na UI (prévia etc.)
# Aqui existe uma possivel falha de quebra de linha causada por "\n" e deve ser checado
NL_FILE = "\n"   # usado no arquivo salvo para evitar linhas em branco no Windows
...
# ---------- Paths & Helpers (compatível com .py e .exe PyInstaller) ----------
def is_frozen() -> bool:
    return getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS")

def app_dir() -> Path:
    """Diretório onde está o .exe (frozen) ou o .py (dev)."""
    if is_frozen():
        return Path(sys.executable).resolve().parent
    return Path(__file__).resolve().parent

# Nome base do INI ao lado do executável/script
INI_BASENAME = "exportador_contagem.ini"

def ini_path() -> Path:
    """Caminho do INI ao lado do executável/script."""
    return app_dir() / INI_BASENAME


# ------------------------------ Logging incremental ------------------------------
LOG_FILE: Path = Path(sys.argv[0]).with_suffix(".log")  # mesmo nome do .exe/.py
def _setup_logging() -> None:
    ...
    if not root.handlers:  # evita duplicar handlers em reexecuções
        ...
        # Mesmo com --noconsole, não atrapalha; em dev ajuda a ver no terminal
...
def _append_log(msg: str) -> None:
    """Escreve uma linha no log incremental do app."""
    try:
        logging.info(msg)
    except Exception:
        pass
...
logging.info("Diretório da aplicação: %s", app_dir())

def excecao_global(exc_type, exc_value, exc_traceback):
    """Captura exceções não tratadas e grava no log."""
    if issubclass(exc_type, KeyboardInterrupt):
        return  # permite encerrar sem stack trace ruidoso
    logging.critical("Exceção não tratada", exc_info=(exc_type, exc_value, exc_traceback))

# Ativar captura global
...
cod_ean,
des_produto,
dta_alteracao as DTAlteracao,
dta_cadastro as dTcadastro,
qtd_estoque_atual as qtd,
val_custo as custo,
val_venda as VR1,
des_marca as marca,
dta_vencimento as vencimento,
des_cor as cor,
des_tamanho as tamanho,
val_venda_dois AS VR2,
flg_envia_balanca as BALANCA,
cod_imposto AS TRIBUTOS,
obs_produto as obs,
ncm,
unidade,
val_venda_promocao AS VRPROMOC,
des_secao AS DEPARTAMENTO
...
def ini_path() -> pathlib.Path:
    """
    Garante que o arquivo .ini fique no mesmo diretório do executável
    quando rodando empacotado (PyInstaller), e no diretório do script
    quando em modo normal (fonte .py).
    """
...
def one_line(text: Any) -> str:
    """Converte texto multi-linha em uma única linha, sem usar literais."""
...
class ExportadorTXTApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Flex container (fonte + colunas + formatação/ações + prévia)
        self.flex = ttk.Frame(self)
        self.flex.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        self.flex.rowconfigure(0, weight=4)  # fonte (file/db)
        self.flex.rowconfigure(1, weight=5)  # colunas
        self.flex.rowconfigure(2, weight=1)  # formatação + ações
        self.flex.rowconfigure(3, weight=0)  # ações (botões)
        self.flex.rowconfigure(4, weight=3)  # prévia
        self.flex.columnconfigure(0, weight=1)

        # ---------------- Fonte de dados (row=0) ----------------
        ...
        # ---------------- Formatação + Ações (row=2) ----------------
        fmt = ttk.Frame(self.flex)
        fmt.grid(row=2, column=0, sticky="ew", padx=10, pady=4)
        ...
        ttk.Checkbutton(fmt, text="Validar EAN no formato EAN13", variable=self.validate_ean13).pack(side=tk.LEFT, padx=(8,0))
        
        ## Barra de ações separada para abrir espaço ao checkbox
        actions = ttk.Frame(self.flex)
        actions.grid(row=3, column=0, sticky="ew", padx=10, pady=(0, 6))
        
        ttk.Button(actions, text="Prévia", command=self.on_preview).pack(side=tk.LEFT)
        ttk.Button(actions, text="Salvar TXT…", command=self.on_save).pack(side=tk.LEFT, padx=(6, 0))

        # ---------------- Prévia (row=4) ----------------
        preview_wrap = ttk.LabelFrame(
            self.flex, text="Prévia do resultado – COD_INTERNO|COD_EAN|DES_PRODUTO"
        )
...
if __name__ == "__main__":
    try:
        _append_log("startup")
        main()
        _append_log("shutdown ok")
    except Exception as exc:
        tb = traceback.format_exc()
        _append_log("EXCEPTION:\n" + tb)
        # Mostra uma msg amigável no build --noconsole
        if messagebox:
            try:
                messagebox.showerror(
                    "ExportadorContagem - erro",
                    f"Ocorreu um erro e foi gravado no log.\n\n{exc}"
                )
            except Exception:
                pass
        # Não relança exceção -> evita fechar seco
