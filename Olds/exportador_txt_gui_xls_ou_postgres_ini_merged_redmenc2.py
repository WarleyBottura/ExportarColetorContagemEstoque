...
-----------------------------------------------------------------------------
O que foi adicionado em relação ao seu script original:
- Suporte a `exportador_contagem.ini` para salvar/carregar:
  - Credenciais do BD (host, porta, banco, usuário, senha)
  - SQL de consulta (multilinha)
  - Variáveis BASE_COL e MANDATORY_PREFIX
- Primeiro uso: se o INI não existir, é criado com padrões e é oferecido abrir para editar.
- Botão **"Salvar .ini"** na área de BD para persistir alterações.
- Leitura do INI com `interpolation=None` (não quebra com `%` em SQL: LIKE '%%ABC%%').
- Mantive **exatamente** o layout e o comportamento dos rádios/frames do seu script.
...
APP_NAME = "Exportador para APP de Contagem – Planilha ou BD PDV (By Bottura)"
...
class ExportadorTXTApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._build_db_frame_contents()  # conteúdo é criado já
        ...
        self.cols_per_block = tk.IntVar(value=3)  # comece com 3 colunas
        ...
        # não expande verticalmente; ocupa só a largura
        ...
        self.preview_txt.pack(fill=tk.BOTH, expand=True)  # dentro do frame pode preencher
        
        font = tkfont.nametofont(self.preview_txt['font'])
        line_px = font.metrics('linespace')
        preview_wrap.configure(height=line_px * 10 + 24)  # ~24 px do título/bordas
        preview_wrap.pack_propagate(False)  # impede crescer/encolher


        # Sincroniza visibilidade inicial (file visível; DB escondido até clicar no rádio)
    ...
    def on_change_source(self):
        ...
        if src == "file":
            ...
            self.db_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=10, pady=(0,6))
    ...
    def _populate_columns_ui(self):
        ...
        ncols = max(1, int(self.cols_per_block.get()))
        total = len(cols_all)
        if total == 0:
            return

        # número de linhas por coluna visual
        rows = math.ceil(total / ncols)

        # deixar cada coluna visual "elástica" na largura
        for j in range(ncols):
            self.inner.columnconfigure(j, weight=1)

        # cabeçalhos por coluna visual (Usar | Coluna | Rótulo)
        for j in range(ncols):
            ...
            # dá elasticidade para label/entry acompanharem a largura
            header.columnconfigure(1, weight=1)
            header.columnconfigure(2, weight=1)

        # itens: distribuímos em "coluna visual" (c) e "linha" (r)
        for idx, col in enumerate(cols_all):
            r = 1 + (idx % rows)      # começa na linha 1 (linha 0 é o cabeçalho)
            c = idx // rows           # avança coluna visual a cada 'rows' itens
            ...
            # nome da coluna ocupa o espaço elástico à esquerda
            ...
            rowf.columnconfigure(1, weight=1)  # nome da coluna expande
            rowf.columnconfigure(2, weight=1)  # rótulo também pode ganhar espaço
