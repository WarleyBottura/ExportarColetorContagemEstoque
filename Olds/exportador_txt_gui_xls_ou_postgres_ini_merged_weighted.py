...
- Reorganização do layout inferior com um container `self.flex` usando **grid** e pesos
  para o redimensionamento vertical proporcional:
  - `mid` (lista de colunas) → peso **1** (linha 0)
  - `db_frame` (Conexão BD) → peso **2** (linha 1)
- A prévia permanece fixa ~10 linhas (altura mínima) e **não** absorve expansão.
- `db_frame` agora é filho de `self.flex` e alterna visibilidade com `grid_remove()`/`grid()`.
...
APP_NAME = "Exportador para APP de Contagem – Planilha ou BD PostgreSQL (By Bottura)"
...
class ExportadorTXTApp(tk.Tk):
    ...
    def on_run_query(self):
        ...
        # <<< ATUALIZA A PRÉVIA AUTOMATICAMENTE >>>
        self.on_preview()
    ...
    def _populate_columns_ui(self):
        ...
        # 4) Monta lista distribuída em N colunas visuais
        import math
